...
**Bill parsing pipeline:** PDF upload → `pdf-parse` text extraction → provider detection → parser plugin → DB insert. Falls back to OCR (`pdftoppm` + `tesseract`) when text extraction fails. The parse+store steps (save PDF → `parseBillPDF` → `mapBillToRows` → insert) are shared via `ingestBillFromBuffer()` in `backend/src/lib/ingest-bill.ts`, used by both the HTTP upload route and the Telegram bot.

**Telegram bot (bill upload):** Users can also add bills by sending a PDF to the Telegram bot. Telegram POSTs updates to `POST /telegram/webhook` (`backend/src/routes/telegram.ts`), verified by the `TELEGRAM_WEBHOOK_SECRET` header. `backend/src/lib/telegram-handler.ts` resolves the sender to an account by matching `message.from.id` against `users.telegramId` (the same key the Login Widget uses — so anyone who has logged in via Telegram is recognized), picks their primary household, downloads the PDF via the hand-rolled Bot API client (`backend/src/lib/telegram-bot.ts`, plain `fetch`), and runs `ingestBillFromBuffer`. The webhook is auto-registered on startup: the URL is `TELEGRAM_WEBHOOK_URL` if set, else derived from an https `FRONTEND_URL` as `${FRONTEND_URL}/api/telegram/webhook`. So in production, setting just `TELEGRAM_BOT_TOKEN` is enough. Manual control via `npm run telegram:webhook` (or `-- delete`).
