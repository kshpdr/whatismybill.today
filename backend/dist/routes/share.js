...
router.delete("/households/:id/share/:token", requireAuth, async (c) => {
    ...
    await db.delete(shareLinks).where(and(eq(shareLinks.token, token), eq(shareLinks.householdId, householdId)));
...
router.get("/share/:token/bills/:billId/pdf", async (c) => {
    ...
    const [bill] = await db.select().from(bills).where(and(eq(bills.id, billId), eq(bills.householdId, link.householdId))).limit(1);
    if (!bill)
