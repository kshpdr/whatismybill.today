...
export function manualEntryToPGEBill(entry) {
    const adj = entry.electricityAdjustments ?? 0;
    const balanceFwd = entry.balanceForward ?? 0;
    const elTotal = round2(entry.electricityDeliveryCost + adj + entry.electricityGenerationCost);
    const elEffectivePrice = entry.electricityUsageKwh > 0
        ? round2((elTotal / entry.electricityUsageKwh) * 100000) / 100000
        : 0;
    const gasEffectivePrice = entry.gasUsageTherms > 0
