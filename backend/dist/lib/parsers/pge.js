...
function parseSummary(text) {
    ...
    const electricDelivery = parseAmt(first(raw, /Current PG&E Electric Delivery Charges\s+\$?([\d,]+\.?\d*)/));
    const electricAdjustments = parseAmt(first(raw, /Electric Adjustments\s+(-?[\d,]+\.?\d*)/));
    const electricGeneration = parseAmt(first(raw, /San Jose Clean Energy Electric Generation Charges\s+([\d,]+\.?\d*)/));
    const gas = parseAmt(first(raw, /Current Gas Charges\s+([\d,]+\.?\d*)/));
    // "Total Amount Due   by MM/DD/YYYY   $NNN.NN" (in-period summary line)
    const totalAmountDue = parseAmt(first(raw, /Total Amount Due\s+by\s+\S+\s+\$?([\d,]+\.?\d*)/));
    const previousBalance = parseAmt(first(raw, /Amount Due on Previous Statement\s+\$?([\d,]+\.?\d*)/));
    // Payments show as negative in the bill text, e.g. "-241.72"
    const paymentsReceived = parseAmt(first(raw, /Payment\(s\) Received Since Last Statement\s+(-?[\d,]+\.?\d*)/));
    const balanceForward = parseAmt(first(raw, /Previous Unpaid Balance\s+\$?([\d,]+\.?\d*)/));
    // currentCharges = charges generated this period only (excludes past-due carry-over)
    const currentCharges = totalAmountDue - Math.max(0, balanceForward);
    return {
        electricDelivery,
        electricAdjustments,
        electricGeneration,
...
function parseElectricDelivery(text) {
    ...
    const periodMatch = raw.match(/(\d{2}\/\d{2}\/\d{4})\s*(?:[-–]|to)\s*(\d{2}\/\d{2}\/\d{4})\s*\((\d+)\s*billing days\)/);
    ...
    const usageTotal = parseFloat(first(raw, /Total Usage\s+([\d.]+)\s*kWh/) || "0");
    const total = parseAmt(first(raw, /Total PG&E Electric Delivery Charges\s+\$?([\d,]+\.?\d*)/));
    return { periodStart, periodEnd, billingDays, usageTotal, total };
}
function parseElectricGeneration(text) {
    const raw = section(text, "Details of San Jose Clean Energy Electric Generation", "Details of Gas Charges");
    // "Total San Jose Clean Energy Electric\nGeneration Charges  $NNN" — label wraps across lines
    const total = parseAmt(first(raw, /Total San Jose Clean Energy Electric\s+Generation\s+Charges\s+\$?([\d,]+\.?\d*)/));
    // Cross-validate kWh usage with delivery page
    const usageCrossCheck = parseFloat(first(raw, /Total Usage\s+([\d.]+)\s*kWh/) || "0");
    // Line items: "Generation - On Peak - Winter   29.469000 kWh @ $0.16939  $4.99"
    const lineItems = [];
    const lineRe = /^(Generation\s*-[^\t\n]+?)\s+[\d.]+\s*kWh\s+@\s+\$[\d.]+\s+\$?([\d,]+\.\d{2})/gm;
    let m;
 ...
 */
function parseElectricBreakdown(text) {
...
function parseGas(text) {
    ...
    const periodEnd = periodMatch ? toISO(periodMatch[2]) : "";
    const billingDays = periodMatch ? parseInt(periodMatch[3]) : 0;
    const usageTotal = parseFloat(first(raw, /Total Usage\s+([\d.]+)\s*Therms/) || "0");
    const total = parseAmt(first(raw, /Total Gas Charges\s+\$?([\d,]+\.?\d*)/));
 ...
 */
function parseGasSegment(segText, periodStart, periodEnd) {
    // "Tier 1 Allowance   44.40 Therms   (30 days x 1.48 Therms/day)"
    const tier1Allowance = parseFloat(first(segText, /Tier 1 Allowance\s+([\d.]+)\s*Therms?/) || "0");
    // "Tier 1 Usage   0.490000Therms @ $2.52299  $1.24"
    // Capture group 1 = Therms quantity, group 2 = dollar amount
    const t1 = segText.match(/Tier 1 Usage\s+([\d.]+)\s*Therms?\s*@\s*\$[\d.]+\s+\$?([\d,]+\.\d{2})/);
    const tier1Usage = t1 ? parseFloat(t1[1]) : 0;
    const tier1Amount = t1 ? parseAmt(t1[2]) : 0;
    const t2 = segText.match(/Tier 2 Usage\s+([\d.]+)\s*Therms?\s*@\s*\$[\d.]+\s+\$?([\d,]+\.\d{2})/);
    ...
    const tax = parseAmt(first(segText, /San Jose Utility Users' Tax[^\n]*?\t([\d,]+\.\d{2})/));
    const franchise = parseAmt(first(segText, /San Jose Franchise Surcharge\s+([\d,]+\.\d{2})/));
    const lineItems = [];
    if (tier1Amount)
        lineItems.push({ label: "Tier 1 Gas", amount: tier1Amount });
    if (tier2Amount)
        lineItems.push({ label: "Tier 2 Gas", amount: tier2Amount });
    if (ppp)
        lineItems.push({ label: "Gas PPP Surcharge", amount: ppp });
    if (tax)
        lineItems.push({ label: "Sales Tax", amount: tax });
    if (franchise)
        lineItems.push({ label: "Franchise Surcharge", amount: franchise });
    return {
        periodStart,
        periodEnd,
 ...
 */
function parseGasSegments(gasText) {
    const headerRe = /(\d{2}\/\d{2}\/\d{4})(?:\s*(?:[-–]|to)\s*(\d{2}\/\d{2}\/\d{4}))?\s+Your Tier Usage/g;
    const headers = [];
    let m;
    ...
    return headers.map((h, i) => {
        const sliceEnd = i + 1 < headers.length ? headers[i + 1].index : gasText.length;
 ...
 */
function aggregateGasLineItems(segments) {
    const totals = {};
...
function parseAccountInfo(text) {
    ...
    const statementDate = toISO(first(text, /Statement Date:\s+(\d{2}\/\d{2}\/\d{4})/));
    const dueDate = toISO(first(text, /Due Date:\s+(\d{2}\/\d{2}\/\d{4})/));
    // Customer name and address from "Service For:" block:
    //   Service For:\n.\nDENIS KOSHELEV\n269 SUNOL ST\nSAN JOSE, CA 95126
    const sfMatch = text.match(/Service For:\s*\n\.\s*\n([^\n]+)\n([^\n]+)\n([^\n]+)/);
 ...
 */
export function extractBillingHistory(text) {
    const entries = [];
    // Each entry: "For M/DD YYYY electric $ NNN.NN gas $ NN.NN"
    const re = /For\s+(\d{1,2}\/\d{2})\s+(\d{4})\s+electric\s+\$\s*([\d.]+)\s+gas\s+\$\s*([\d.]+)/g;
    let m;
...
// ─── Main parser ──────────────────────────────────────────────────────────────
export function parsePGEText(text) {
    const flags = [];
    ...
    // ─── Validation ─────────────────────────────────────────────────────────────
    if (!generation.total)
        flags.push("missing_generation");
    if (!delivery.usageTotal)
        flags.push("missing_usage");
    if (!delivery.periodStart)
        flags.push("partial_bill");
    if (!gas.periodStart)
        flags.push("partial_bill");
    // Cross-validate electricity usage between delivery and generation pages
    if (delivery.usageTotal > 0 &&
    ...
    const computedCurrent = summary.electricDelivery +
        summary.electricAdjustments +
        summary.electricGeneration +
        summary.gas;
    if (summary.currentCharges > 0 &&
    ...
    const delivTotal = delivery.total || summary.electricDelivery;
    const genTotal = generation.total || summary.electricGeneration;
    ...
    const elEffectivePrice = delivery.usageTotal > 0
        ? round2(elTotal / delivery.usageTotal * 100000) / 100000
        : 0;
    // ─── Derived gas values ───────────────────────────────────────────────────────
    const gasTotal = gas.total || summary.gas;
    const gasEffectivePrice = gas.usageTotal > 0
...
export function isPGEBill(text) {
